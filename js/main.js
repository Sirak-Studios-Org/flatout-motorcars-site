...
(function () {
  ...
  // CRM relay (Zo) — mirrors the submission into Airtable; email still goes via Web3Forms native POST
  var CRM_RELAY = 'https://sirak.zo.space/api/webhooks/flatout-drive-request';
  var FIELDS = ['name', 'email', 'phone', 'build', 'experience', 'fastest_car', 'timeline', 'date_1', 'date_2', 'date_3', 'message'];
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      // Send to the CRM relay first, then submit natively to Web3Forms (for the email).
      // A fail-safe timer guarantees the native submit runs even if the relay is slow or down.
      e.preventDefault();
      var payload = {};
      FIELDS.forEach(function (n) { payload[n] = val(n); });
      var submitted = false;
      var go = function () { if (!submitted) { submitted = true; form.submit(); } };
      try {
        fetch(CRM_RELAY, {
          method: 'POST',
